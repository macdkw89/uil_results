...
def click_results_button():
    get_results_button = driver.find_element(
        by=By.CLASS_NAME, value="btn-success")
...
def form_completer(year, region, contest_item):
    ...
    contest_menu_contents = contest_menu.find_elements(
...
def scraper_loop():
    show_all_button = driver.find_element(
        by=By.NAME, value="DataTables_Table_0_length")
...
def convert_to_dataframe():
    ...
    df.to_csv(
        f"full_run/texas_music_forms_{year}_{region}_{contest_item}_band.csv")
